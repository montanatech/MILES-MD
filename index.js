...
const ownerNumber = ['263781856195']
...
if (!fs.existsSync(__dirname + '/sessions/creds.json')) {
  ...
  const sessdata = config.SESSION_ID.replace("Miles;;;", '')
  ...
  try {
    ...
    conn.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        if (shouldReconnect) setTimeout(() => connectToWA(), 5000)
          ...
          fs.readdirSync("./miles/").forEach((plugin) => {
            if (path.extname(plugin).toLowerCase() === ".js") require("./popkid/" + plugin)
          })
          console.log('[ 🪀 ] Bot connected to WhatsApp ✅')

          let up = `╔════════════════╗\n║ 🤖 ▰𝗖𝗢𝗡𝗡𝗘𝗖𝗧𝗘𝗗▰\n╠════════════════╣\n║ 🔑 PREFIX  : ${config.PREFIX}\n║ 👨‍💻 DEV     : Miles-MD\n║ 📞 DEV NO : 263781856195\n╚════════════════╝`;
          conn.sendMessage(conn.user.id, { image: { url: `https://files.catbox.moe/aq85vi.jpg` }, caption: up })
    ...
    setInterval(async () => {
        if (config.AUTO_BIO === "true" && conn?.user) {
            const date = new Date().toLocaleDateString('en-KE', { timeZone: 'Africa/Zimbabwe' });
            const time = new Date().toLocaleTimeString('en-KE', { timeZone: 'Africa/Zimbabwe', hour12: false });
            const bioText = `ᴍɪʟᴇs xᴍᴅ ʙᴏᴛ 🤖 ɪs ʟɪᴠᴇ ɴᴏᴡ\n📅 ${date}\n⏰ ${time}`;
...
app.get("/", (req, res) => res.send("ᴍɪʟᴇs-ᴍᴅ ɪs ɴᴏᴡ ᴀᴄᴛɪᴠᴇ"));
